...
* **硬约束路:实体投影 + 规则引擎(不走 top_k)。**
  关键点——硬约束**绝不用相似度检索取证据**。给定 `subject_entity`,直接按 entity_id 从 World State Store 拉**全量相关历史**(全序结构,SQL `WHERE entity_id=? ORDER BY change_chapter`),再喂给确定性 validator。理由:相似度检索是概率召回,会漏(top_k 截断 = 漏召回 = 漏报),而境界越级/缺失边这类问题恰恰要求"看全所有历史才能判定"。例如检测"角色泄露了不该知道的信息",必须拿到该角色**完整**的 `knowledge_edges` 集合(按 `knower_entity_id` 查),缺一条边就误判。
  ...
  def gather_hard_context(claim: Claim, conn) -> "EntityProjection":
      ...
      return EntityProjection(
          power_log   = sql_all(conn,
              "SELECT * FROM character_power_log WHERE entity_id=? AND change_chapter<=? ORDER BY change_chapter",
              ent, claim.chapter),
          knowledge   = sql_all(conn,
              "SELECT * FROM knowledge_edges WHERE knower_entity_id=? AND learned_chapter<=?",
              ent, claim.chapter),
          timeline    = sql_all(conn,
              "SELECT * FROM timeline_events WHERE story_time_start IS NOT NULL ORDER BY story_time_start"),
          item_log    = sql_all(conn,
              "SELECT * FROM item_log WHERE (item_entity_id=? OR from_owner_id=? OR to_owner_id=?) AND change_chapter<=? ORDER BY change_chapter",
              ent, ent, ent, claim.chapter),
...
**模型**:`power_ranks` 是**有序枚举**(`rank_order` 整数全序,如 炼气=10 < 筑基=20 < 金丹=30 …);`character_power_log(entity_id, change_chapter, rank_order, rank_id, change_type, author_flag)` 记录每次境界变化。`change_type` 合法值:`breakthrough`/`injury_drop`/`seal`/`unseal`/`init`。
...
```python
# LEGAL_DOWN_REASONS: change_type 合法跌境值(对应 character_power_log.change_type)
LEGAL_DOWN_REASONS = {"injury_drop", "seal", "self_cripple"}

def validate_power_monotonicity(claims, world, conn) -> list[Issue]:
    issues = []
    # rank_order_map: {(system_name, rank_name): rank_order} — 按体系+境界名索引
    rank_order_map = world.rank_order_map
    for c in [c for c in claims if c.ctype == ClaimType.POWER_LEVEL]:
        ent = c.subject_entity
        history = world.power_history(ent)            # 全量,已 ORDER BY change_chapter
        cur_label = c.payload["rank_label"]
        system = c.payload.get("system_name", "default")
        cur = rank_order_map.get((system, cur_label))
        if cur is None:
        ...
        if not history:
            continue  # 首次出现,无历史可比
        prev = history[-1]
        prev_order = prev["rank_order"]   # 直接使用 rank_order 整数,无需 label 查表
        # —— 下降检测 ——
        if cur < prev_order:
            if c.payload.get("change_type") in LEGAL_DOWN_REASONS \
            ...
            issues.append(Issue(code="POWER_REGRESSION", severity="critical",
                kind="hard", claim_id=c.claim_id, chapter=c.chapter,
                message=(f"{ent} 境界从『{prev['rank_id']}』(第{prev['change_chapter']}章)"
                         f"倒退到『{cur_label}』,无合法跌境标注"),
                evidence_refs=[prev["ref"], c.claim_id],
                suggested_fix="若为重伤跌境,请在草稿打 change_type=injury_drop"))
        # —— 越级检测 ——
            ...
            if not justified and "intentional_leap" not in c.exempt_tags:
                issues.append(Issue(code="POWER_LEVEL_SKIP", severity="major",
                    kind="hard", claim_id=c.claim_id, chapter=c.chapter,
                    message=(f"{ent} 从『{prev['rank_id']}』直跳『{cur_label}』,"
...
**模型**:`knowledge_edges(knower_entity_id, secret_key, learned_chapter, source, public_from_chapter, secrecy_level)` 是**只增**的知情者图(完整 DDL 含 `public_from_chapter INTEGER NULL`、`secrecy_level TEXT` 两列详见第 2 节);角色 `ent` 在第 N 章的知情集 = `{secret_key | edge(ent, secret_key).learned_chapter <= N}`。

**规则**:对每个 `knowledge` 类 claim,若 `act ∈ {reveal, reference, act_on}` 涉及 `info_key`(对应表列 `secret_key`),而 `secret_key ∉ knowledge_set(ent, N)`,则报 `KNOWLEDGE_LEAK`(critical)——除非该 info 已转为公共知识(`is_public(info, N)` ⟺ `public_from_chapter IS NOT NULL AND public_from_chapter <= N`;多边取 `MIN(public_from_chapter)`,详见第 2 节)或带 `planted_misdirection`/`unreliable_narrator` 豁免(§4.5)。
...
**模型**:`timeline_events(event_key, story_time_start, story_time_end, source_chapter)`,**绝对 story_time_start/story_time_end** 是唯一真相;相对时间("三天后")在**入库时即解析为绝对区间**(见下 `resolve_relative`),不在校验期才算。`networkx` 拓扑排序做**环检测**(A 在 B 后、B 在 C 后、C 又在 A 后)。

**入库解析(写时,非校验期)**:

```python
def resolve_relative(rel_expr: str, rel_to_event: str, world) -> tuple[ISO, ISO]:
    """'三天后' + 锚点事件 → 绝对区间 [lo, hi];存入 timeline_events.story_time_start/story_time_end。"""
    anchor = world.event_abs(rel_to_event)          # 锚点的绝对区间
    delta = parse_cn_duration(rel_expr)             # '三天后'->timedelta(days=3) 等
    return (anchor.story_time_start + delta, anchor.story_time_end + delta)
```
...
def validate_timeline(claims, world, conn) -> list[Issue]:
    ...
    for c in [c for c in claims if c.ctype == ClaimType.TIMELINE]:
        if c.payload.get("rel_expr"):
            story_time_start, story_time_end = resolve_relative(c.payload["rel_expr"],
                                                                 c.payload["rel_to_event"], world)
        else:
            story_time_start = story_time_end = c.payload.get("abs_story_time")
        ev = c.payload["event_key"]
        if (clash := world.abs_clash(ev, story_time_start, story_time_end)):
            issues.append(Issue(code="TIMELINE_CONTRADICTION", severity="major",
                kind="hard", claim_id=c.claim_id, chapter=c.chapter,
                message=f"事件『{ev}』绝对时间[{story_time_start},{story_time_end}] 与既有[{clash}]不相容",
                evidence_refs=[c.claim_id]))
    return issues
```
...
**模型**:`item_ownership(item_entity_id, quantity)`(当前快照,投影产物)+ `item_log(item_entity_id, from_owner_id, to_owner_id, change_type, quantity_delta, change_chapter)`(只增账本)。
...
def validate_item_inventory(claims, world, conn) -> list[Issue]:
    ...
    for c in [c for c in claims if c.ctype == ClaimType.ITEM_OWNERSHIP]:
        ...
        op, qty = c.payload["op"], c.payload["quantity_delta"]
        held = world.item_qty(owner, item)          # = replay(item_log) 截至 N-1,用 item_entity_id
...
**模型**:`foreshadow(id, label, planted_chapter, due_chapter, state, reinforced_chapters)`,状态机 `planted→reinforced→misled→paid_off→overdue`。

**规则**:
* 本章 claim 触及伏笔(`reinforce`/`pay_off`/`mislead`)→ 推进状态机,非法迁移报 `FORESHADOW_ILLEGAL_STATE`(minor)。
* 扫描:`state ∉ {paid_off, abandoned}` 且 `chapter > due_chapter` → **自动置 `overdue`** 并产 `FORESHADOW_OVERDUE`(major,提醒"挖了坑没填")。注意:这是工艺/追更力问题但用确定性表达,issue 进 continuity_check 流;与第 5 节工艺层 `foreshadow` 表共用同一张表。
    ...
    for c in [c for c in claims if c.ctype == ClaimType.FORESHADOW]:
        ...
        if not fs.can_transition(c.payload["act"]):
            issues.append(Issue(code="FORESHADOW_ILLEGAL_STATE", severity="minor",
                ...
                message=f"伏笔『{fs.label}』状态{fs.state}不可执行{c.payload['act']}",
                evidence_refs=[c.claim_id]))
    # 2) 全表到期扫描(主动),写回 state=overdue(append 一条状态变更,非物理改)
    for fs in world.all_open_foreshadows():
        if fs.state not in ("paid_off", "abandoned") and n > fs.due_chapter:
            ...
            issues.append(Issue(code="FORESHADOW_OVERDUE", severity="major",
                ...
                message=(f"伏笔『{fs.label}』第{fs.planted_chapter}章埋下,"
                         f"deadline 第{fs.due_chapter}章已过仍未回收,自动置 overdue"),
                evidence_refs=[fs.id]))
    return issues
```
