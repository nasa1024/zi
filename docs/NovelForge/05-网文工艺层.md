...
表结构以 §02.6 为权威，本节不重复 DDL 定义。

> `beats` / `chapter_cards` / `character_cards` / `character_arc_stages` / `pacing_state` 的完整 DDL（含索引、CHECK 约束、字段注释）见 §02.6。本节仅说明这些表的**语义用途**：`beats` 落 beat sheet 逐行契约；`chapter_cards` 记逐章节奏指标与钩子；`character_cards` + `character_arc_stages` 做角色双轨建模；`pacing_state` 供 `PacingController` 维护跨章张力曲线。`foreshadow` 表同样由第 2 节定义，本节仅引用其 `status`（planted→reinforced→misled→paid_off→overdue）与 `semantic_tags`（含"被轻视"）字段。
